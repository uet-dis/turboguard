#!/usr/bin/env python3
"""CIC-IDS2017 — Data preparation.

Loads raw CIC-IDS2017 data via the registry handler, splits into Sectors A/B/C,
and saves everything to a timestamped run folder.

Usage::

    python -m datasets.cic2017.prepare --data-dir ./data/cic2017
    python -m datasets.cic2017.prepare --data-dir ./data/cic2017 --scaler standard
"""
...
def main():
    parser = argparse.ArgumentParser(description="CIC-IDS2017 — Prepare data")
    ...
    handler = get_handler("cic2017")
    header("Prepare Data", dataset="cic2017")
    ...
    run_dir = create_run_dir(base, "cic2017", "prepare")
    prepared = prepare_locked_artifacts(
        run_dir, "cic2017", X, y, args.seed, preprocessing_audit
    )
