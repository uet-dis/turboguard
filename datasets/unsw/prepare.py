#!/usr/bin/env python3
"""UNSW — Data preparation.

Loads raw UNSW data via the registry handler, splits into Sectors A/B/C,
and saves everything to a timestamped run folder.

Usage::

    python -m datasets.unsw.prepare --data-dir ./data/CIC-UNSW
    python -m datasets.unsw.prepare --data-dir ./data/CIC-UNSW --scaler standard
"""
...
def main():
    parser = argparse.ArgumentParser(description="UNSW — Prepare data")
    ...
    handler = get_handler("unsw")
    header("Prepare Data", dataset="unsw")
    ...
    run_dir = create_run_dir(base, "unsw", "prepare")
    prepared = prepare_locked_artifacts(
        run_dir, "unsw", X, y, args.seed, preprocessing_audit
    )
