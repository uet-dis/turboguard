#!/usr/bin/env python3
"""CIC-IDS2017 — Per-Signal AUC and Calibration Analysis for TurboGuard.
...
def main():
    parser = argparse.ArgumentParser(description="CIC-IDS2017 — Signal AUC Analysis")
    ...
    header("Signal AUC Analysis", dataset="cic2017")
    ...
    for label, stem in THESIS_ATTACKS:
        ...
        if matches:
            ...
            console.print(
                f"    [green]✓[/green] {label}: {n_adv:,} / {len(X_adv_full):,} samples ({matches[0].name})"
            )
        else:
    ...
    if not scenarios:
        console.print(
            "  [yellow]No matching files for thesis table — loading all available .pt files[/yellow]"
        )
    ...
    # ── Save report ──────────────────────────────────────────────────────
    out = create_run_dir(Path("results"), "cic2017", "signal_auc")
    report = {
