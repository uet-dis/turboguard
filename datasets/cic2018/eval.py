#!/usr/bin/env python3
"""CIC2018 — Evaluate TurboGuard against generated attacks.

Usage::

    python -m datasets.cic2018.eval \\
...
def main():
    parser = argparse.ArgumentParser(description="CIC2018 — Evaluate")
    ...
    header("Evaluate Benchmark (Sector C)", dataset="cic2018")
    ...
    for model_name, model in models.items():
        ...
        results["Clean"][model_name] = {
            "EDR": float('nan'), "clean_ADR": adr_clean, "ADR": adr_clean, 
            "FPR": fpr, "F1": f1, "TP": tp, "FN": fn, "TN": tn, "FP": fp,
    ...
    for af in attack_files:
        ...
        for model_name, model in models.items():
            ...
            results[name][model_name] = {
                "EDR": edr, "ADR": adr, "FPR": fpr, "F1": f1,
                "TP": tp, "FN": fn, "TN": tn, "FP": fp,
                "Accuracy": acc, "Precision": precision, "Recall": recall,
                "Filter-EDR": filter_edr, "Clf-EDR": clf_edr
            }
    ...
    out = create_run_dir(base, "cic2018", "eval")

    with open(out / "report.json", "w") as f:
        json.dump(results, f, indent=2)

    lines = ["Benchmark Evaluation — CIC2018", "=" * 60]
