#!/usr/bin/env python3
"""KDD — Train TurboGuard + baseline models.

Usage::

    python -m datasets.kdd.train --run-dir results/unsw/<ts>_prepare
"""
...
def main():
    parser = argparse.ArgumentParser(description="KDD — Train models")
    ...
    if not args.skip_baseline:
        header("Train Baselines", dataset="kdd")
        bl_dir = create_run_dir(base, "kdd", "baseline")
    ...
    if not args.skip_turboguard:
        header("Train TurboGuard", dataset="kdd")
        tg_dir = create_run_dir(base, "kdd", "turboguard")
