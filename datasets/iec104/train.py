#!/usr/bin/env python3
"""IEC104 — Train TurboGuard + baseline models.

Usage::

    python -m datasets.iec104.train --run-dir results/unsw/<ts>_prepare
"""
...
def main():
    parser = argparse.ArgumentParser(description="IEC104 — Train models")
    ...
    if not args.skip_baseline:
        header("Train Baselines", dataset="iec104")
        bl_dir = create_run_dir(base, "iec104", "baseline")
    ...
    if not args.skip_turboguard:
        header("Train TurboGuard", dataset="iec104")
        tg_dir = create_run_dir(base, "iec104", "turboguard")
