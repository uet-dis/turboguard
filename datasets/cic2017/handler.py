"""CIC-IDS2017 dataset handler.

Loads the preprocessed parquet file (output of ``preprocess.py``),
selects the 62 CICFlowMeter features, and applies scaling with binary
features kept as 0/1.
"""
...
@register_dataset
class CIC2017Handler(DatasetHandler):
    """CIC-IDS2017 dataset (62 features, binary label).

    Expects ``cic2017_clean.parquet`` in the data directory.
    """
    ...
    def load(
        ...
        """Loads CIC-IDS2017 from preprocessed parquet.
        ...
        Args:
            data_dir: Path containing ``cic2017_clean.parquet``.
            scaler_type: ``"minmax"`` or ``"standard"``.

        Returns:
            Tuple of (X, y, scaler).
        """
        df = pd.read_parquet(f"{data_dir}/cic2017_clean.parquet")
        n_loaded = len(df)
