#!/usr/bin/env python3
"""IEC104 — Data preparation.

Loads raw IEC104 data via the registry handler, splits into Sectors A/B/C,
and saves everything to a timestamped run folder.

Usage::

    python -m datasets.iec104.prepare --data-dir ./data/CIC-IEC104
    python -m datasets.iec104.prepare --data-dir ./data/CIC-IEC104 --scaler standard
"""
...
def main():
    parser = argparse.ArgumentParser(description="IEC104 — Prepare data")
    ...
    handler = get_handler("iec104")
    header("Prepare Data", dataset="iec104")
    ...
    run_dir = create_run_dir(base, "iec104", "prepare")
    prepared = prepare_locked_artifacts(
        run_dir, "iec104", X, y, args.seed, preprocessing_audit
    )
