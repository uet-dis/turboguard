#!/usr/bin/env python3
"""CIC-IDS2017 — Explainability and Feature Importance Analysis for TurboGuard.

Usage::

    python -m datasets.cic2017.explain \
...
def main():
    parser = argparse.ArgumentParser(description="CIC-IDS2017 — Explainability Analysis")
    ...
    parser.add_argument(
        "--n-samples", type=int, default=500, help="Number of samples for SHAP"
    )
    ...
    if args.n_samples <= 0:
        ...
        n_ben, n_atk = (
            min(len(benign_idx), args.n_samples // 2),
            min(len(attack_idx), args.n_samples // 2),
        )
    ...
    with console.status(
        "[bold green]Computing SHAP Values (this may take 2-5 minutes)...[/bold green]"
    ):
        shap_v = explainer.shap_values(test_data)
    if isinstance(shap_v, list):
        sv = np.array(shap_v[1])
    elif isinstance(shap_v, np.ndarray) and shap_v.ndim == 3:
        sv = shap_v[:, :, 1]
    else:
        sv = np.array(shap_v)
    ...
    raw_imp, recon_imp = (
        mean_abs_shap[:input_dim].sum(),
        mean_abs_shap[input_dim : 2 * input_dim].sum(),
    )
    res_imp, sig_imp = (
        mean_abs_shap[2 * input_dim : 3 * input_dim].sum(),
        mean_abs_shap[3 * input_dim :].sum(),
    )
    ...
    for tree in tg._iso_forest.estimators_:
        if len(tree.feature_importances_) == 6:
            if_imp += tree.feature_importances_
    ...
    group_data = [
        {
            "Group": "6 Codebook Signals (\u03c3)",
            "SHAP %": f"{sig_imp/total_shap*100:5.1f}%",
            "Function": "VQ-VAE topology signals",
        },
        {
            "Group": "Raw input (x)",
            "SHAP %": f"{raw_imp/total_shap*100:5.1f}%",
            "Function": "Cloud NIDS features",
        },
        {
            "Group": "Reconstruction (\u1e8d)",
            "SHAP %": f"{recon_imp/total_shap*100:5.1f}%",
            "Function": "Manifold boundary",
        },
        {
            "Group": "Residual (|x - \u1e8d|)",
            "SHAP %": f"{res_imp/total_shap*100:5.1f}%",
            "Function": "Recon error signal",
        },
    ]
    results_table(
        "Table 1: Feature Group SHAP Importance",
        sorted(group_data, key=lambda x: -float(x["SHAP %"][:-1])),
        ["Group", "SHAP %", "Function"],
    )
    ...
    for i, name in enumerate(SIGNAL_NAMES):
        sig_data.append(
            {
                "Signal": name,
                "Magnitude": f"{sig_shap_vals[i]:.4f}",
                "% within Signals": f"{sig_shap_vals[i]/sig_imp*100:5.1f}%",
            }
        )
    results_table(
        "Table 2: Individual Signal SHAP Ranking",
        sorted(sig_data, key=lambda x: -float(x["Magnitude"])),
        ["Signal", "Magnitude", "% within Signals"],
    )
    ...
    for i, name in enumerate(SIGNAL_NAMES):
        if_data.append(
            {
                "Signal": name,
                "IF Importance (%)": f"{if_imp[i]*100:5.1f}%",
                "Distribution Bar": "\u2588" * int(if_imp[i] * 50),
            }
        )
    results_table(
        "Table 3: Isolation Forest Importance",
        sorted(if_data, key=lambda x: -float(x["IF Importance (%)"][:-1])),
        ["Signal", "IF Importance (%)", "Distribution Bar"],
    )

    out = create_run_dir(Path("results"), dataset_name, "explain")
    report = {
        "groups": {"signals": sig_imp / total_shap, "raw": raw_imp / total_shap},
        "signals_shap": {
            n: float(sig_shap_vals[i]) for i, n in enumerate(SIGNAL_NAMES)
        },
    }
    with open(out / "explainability_report.json", "w") as f:
